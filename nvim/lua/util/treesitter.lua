...
function M.check()
    ...
    local have_cc = vim.env.CC ~= nil or have("cc") or have("cl")
...
function M.build(cb)
    M.ensure_treesitter_cli(function(_, err)
        ...
        if ok then
            ...
            LazyUtil.error(lines, { title = "Treesitter" })
        end
    end)
end
...
function M.ensure_treesitter_cli(cb)
    ...
    mr.refresh(function()
        ...
        if not p:is_installed() then
            LazyUtil.info("Installing `tree-sitter-cli` with `mason.nvim`...")
            p:install(
                nil,
                vim.schedule_wrap(function(success)
                    if success then
                        LazyUtil.info("Installed `tree-sitter-cli` with `mason.nvim`.")
                        cb(true)
                    else
